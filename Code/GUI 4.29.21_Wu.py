...
# load clean data
#import pandas as pd
project = pd.read_csv('https://raw.githubusercontent.com/JichongWu/Final-Project-Group8/main/Data/project_J.Pai.csv',sep=',')
...
#=========================================================
# Decision Tree code
feature_list = ['Weather','Light Condition','Road Surface','Roadway Grade','Vehicle Type','Roadway Alignment', 'Vehicle Year','Gender','Age']
...
class DecisionTree(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("background-color: pink")        # background color
...
class TotalCount(QMainWindow):
    ...
    def initUi(self):
        self.setWindowTitle(self.Title)
        self.main_widget = QWidget(self)
        self.statusBar().setStyleSheet("background-color: black")  # status bar
...
class AGE(QMainWindow):
    def __init__(self):
        ...
        self.ax1.bar(np.arange(2),project['GENDER'], color='turquoise', label='No Rollover')
        self.ax1.legend()
...
class FeatureVRollover(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(self.Title)
        self.resize(900, 900)
        self.statusBar().setStyleSheet("background-color: lightcyan")  # status bar
        self.setStyleSheet("background-color: white")  # background color
        self.main_widget = QWidget(self)
        # chart
        self.fig = Figure()
        ...
        self.dropdown.addItems(['YEAR','WEATHER_GROUP','LIGHT_GROUP','SURFACE_GROUP','GRADE_GROUP','ALIGH_GROUP','VHICLE_TYPE','MY_GROUP','GENDER','AGE'])
        self.dropdown.setStyleSheet("background-color: lightcyan")
    ...
    def update(self):
        ...
        if self.checkbox.isChecked():
            self.checkbox.triggered.connect(self.age)
...
class DfView(QAbstractTableModel):
    send_fig = pyqtSignal(str)

    def __init__(self, data):
        super(QAbstractTableModel).__init__()
        self._data = data
...
class CarRollover(QMainWindow):
    ...
    def initUI(self):
        ...
        ## df view button
        dfButton = QAction('df', self)
        dfButton.setStatusTip('df')
        dfButton.triggered.connect(self.dfview)
        EDAMenu.addAction(dfButton)
