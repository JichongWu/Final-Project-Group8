...
weather_map = {1:'CLEAR/NORMAL', 2:'RAIN/SLEET', 3:'RAIN/SLEET', 4:'SNOW', 5:'FOG/CLOUDY', 6:'WINDY', 7:'WINDY', 
               8:'NA', 10:'FOG/CLOUDY', 11:'SNOW', 12:'RAIN/SLEET', 98:'MISSING', 99:'MISSING'}
acc_veh_per_copy['WEATHER_GROUP'] = acc_veh_per_copy['WEATHER'].map(weather_map)

# Group light condition
light_map = {1:'LIGHT', 2:'DARK', 3:'LIGHT', 4:'DAWN/DUSK', 5:'DAWN/DUSK', 6:'NA', 7:'NA', 8:'MISSING', 9:'MISSING'}
...
vsurcond_map = {0:'NA', 1:'DRY', 2:'WET', 3:'WET', 4:'WET', 5:'DRY', 6:'WET', 7:'OIL/SLUSH/MUD', 8:'NA', 10:'OIL/SLUSH/MUD', 
                11:'OIL/SLUSH/MUD', 98:'MISSING', 99:'MISSING'}
acc_veh_per_copy['SURFACE_GROUP'] = acc_veh_per_copy['VSURCOND'].map(vsurcond_map)

# Group roadway grade
vprofile_map = {0:'NA', 1:'LEVEL', 2:'GRADE', 3:'GRADE', 4:'GRADE', 5:'GRADE', 6:'GRADE', 8:'MISSING', 9:'MISSING'}
acc_veh_per_copy['GRADE_GROUP'] = acc_veh_per_copy['VPROFILE'].map(vprofile_map)

# Group roadway alignment
valign_map = {0:'NA', 1:'STRAIGHT', 2:'CURVE', 3:'CURVE', 4:'CURVE', 8:'MISSING', 9:'MISSING'}
...
model_year_value = ['1989-2007','2008-2010', '2011-2019', 'MISSING', 'MISSING']
...
# Driver's gender
gender_map = {1:'MALE', 2:'FEMALE', 8:'MISSING', 9:'MISSING'}
...
age_group_value = ['16-20','21-24', '25-34', '35-44', '45-54', '55-64', '65-74', '> 74','NA','MISSING','MISSING']
acc_veh_per_copy['AGE_GROUP'] = np.select(condition2, age_group_value)

# Missing percentage in each feature

acc_veh_per_copy.column = ['WEATHER_GROUP','LIGHT_GROUP','SURFACE_GROUP','GRADE_GROUP','ALIGN_GROUP','MY_GROUP','GENDER']

# Weather Condition
print('Distribution of Weather Condition')
s = acc_veh_per_copy.WEATHER_GROUP
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))

# Light\n Condition
print('\nDistribution of Light Condition')
s = acc_veh_per_copy.LIGHT_GROUP
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))
...
s = acc_veh_per_copy.SURFACE_GROUP
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))
...
print('\nDistribution of Roadway Grade Condition')
s = acc_veh_per_copy.GRADE_GROUP
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))

# Roadway Alignment Condition
print('\nDistribution of Roadway Alignment Condition')
s = acc_veh_per_copy.ALIGN_GROUP
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))
...
# GENDER
print("\nDistribution of Driver's Gender")
s = acc_veh_per_copy.GENDER
counts = s.value_counts()
percent = s.value_counts(normalize=True)
percent100 = s.value_counts(normalize=True).mul(100).round(1).astype(str) + '%'
print(pd.DataFrame({'per100': percent100}))

# List of missing percentage in each feature
missing = [('4.8%'),('0.7%'),('1.2%'),('7.6%'),('2.3%'),('3.9%'),('3.7%')]
            
# Create DataFrame object from a list of tuples
df = pd.DataFrame(missing, columns = ['Missing Percentage'], index=['Weather Condition', 'Light Condition', 'Roadway Surface',
                                                         'Roadway Grade','Roadway Alignment','Model Year',"Driver's Gender"])
print(df)

project=acc_veh_per_copy[(acc_veh_per_copy.WEATHER_GROUP != 'NA') & (acc_veh_per_copy.WEATHER_GROUP != 'MISSING') & 
                         (acc_veh_per_copy.LIGHT_GROUP != 'NA') & (acc_veh_per_copy.LIGHT_GROUP != 'MISSING') &
                         (acc_veh_per_copy.SURFACE_GROUP != 'NA') & (acc_veh_per_copy.SURFACE_GROUP != 'MISSING') &
                         (acc_veh_per_copy.GRADE_GROUP != 'NA') & (acc_veh_per_copy.GRADE_GROUP != 'MISSING') &
                         (acc_veh_per_copy.ALIGN_GROUP != 'NA') & (acc_veh_per_copy.ALIGN_GROUP != 'MISSING') &
                         (acc_veh_per_copy.VEHICLE_TYPE != 'NA') & (acc_veh_per_copy.VEHICLE_TYPE != 'MISSING') &
                         (acc_veh_per_copy.MY_GROUP != 'NA') & (acc_veh_per_copy.MY_GROUP != 'MISSING') &
                         (acc_veh_per_copy.GENDER != 'NA') & (acc_veh_per_copy.GENDER != 'MISSING') &
                         (acc_veh_per_copy.AGE_GROUP != 'NA') & (acc_veh_per_copy.AGE_GROUP != 'MISSING')
                        ]
# Shape of data
print(project.shape)
